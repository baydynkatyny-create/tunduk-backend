...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (lastUserMsg && isBlockedPrompt(lastUserMsg.content)) {
      return res.status(400).json({ error: "Бул сурам эрежелерге каршы келет. Башка суроо жазыңыз." });
    }
...
app.post("/api/analyze-file", async (req, res) => {
  try {
    ...
    if (question && isBlockedPrompt(question)) {
      return res.status(400).json({ error: "Бул сурам эрежелерге каршы келет." });
    }
