...
function buildSpeechInput(text, style) {
  if (!style) style = detectSpeechStyle(text);
  if (style === "poem") {
    const formatted = formatPoemForSpeech(text);
    return (
      "[Стиль: кыргыз акындарынын салттуу декламациясы — Акбар Кубанычбеков " +
      "сыяктуу сахнада ырдын мазмунун бүт денеси, үнү менен жеткирген акын " +
      "окуган стилде]\n" +
      "Так жөнөкөй окубай, СЦЕНАДАН ЫР АЙТКАН АКЫНДАЙ окуп бер:\n" +
      "- Ар куплеттин башында үндү бир аз көтөр, толкундануу менен баштап, " +
      "куплеттин аягына карай интонацияны бекемдеп, чечкиндүү түшүр.\n" +
      "- Маанилүү сөздөрдү (сезим, ат мекен, эне, тагдыр, кыргыз сыяктуу " +
      "күчтүү маанидеги сөздөрдү) созуп, басым коюп, обончо айт.\n" +
      "- Ар сап аягында так, курч дем ал; куплеттер аралыгында бир аз " +
      "узунураак, драмалык тынс.\n" +
      "- Ыргакты так карма — ыр сыяктуу ритмдүү, ыкчам эмес, шашылбаган " +
      "бирок бош да эмес темп менен.\n" +
      "- Үн ички жалындуулук, сыймык жана эмоция менен толгон болсун — " +
      "жөн эле маалымат окуган робот үнү эмес, жүрөктөн чыккан үн.\n\n" +
      formatted
    );
  }
  if (style === "tale") {
...
async function synthesizeFullSpeech(text) {
  const style = detectSpeechStyle(text);
  const chunks = splitTextForTTS(text);
  const audioParts = [];

  for (const chunk of chunks) {
    const speechInput = buildSpeechInput(chunk, style);
...
app.post("/api/speak", async (req, res) => {
  try {
    ...
    let audioBase64;
    try {
      audioBase64 = await synthesizeFullSpeech(text);
    } catch (e) {
