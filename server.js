...
function detectSpeechStyle(text) {
  ...
  const looksLikePoem = isLineBroken && avgLineLen <= 45;
...
function buildSpeechInput(text) {
...
app.post("/api/speak", async (req, res) => {
  try {
    ...
    const speechInput = buildSpeechInput(trimmedText);

    let data;
    try {
      data = await geminiFetch("gemini-3.1-flash-tts-preview", {
