...
// .env файлында GEMINI_API_KEY (милдеттүү), GEMINI_API_KEY_2, GEMINI_API_KEY_3 жана GEMINI_API_KEY_4 (каалосу боюнча) кошсоңуз болот.
...
async function generateImageDataUrl(prompt) {
  const data = await geminiFetch("gemini-3.1-flash-image", {
    contents: [{ parts: [{ text: prompt }] }],
  });
  const part = data.candidates?.[0]?.content?.parts?.find((p) => p.inlineData);
